...
Bare-metal GPU nodes (BM.GPU.B4.8, BM.GPU.H100.8) with RDMA. Two options: hostNetwork or SR-IOV VFs.

For the full guide with explanations, see [deploying-slinky-on-oke.md](deploying-slinky-on-oke.md).

---

## Prerequisites

### Fix CRI-O memlock (required for both approaches)
...
Uses PMIx images for `srun --mpi=pmix`. For `sbatch` + `mpirun` with stock images, see [full guide](deploying-slinky-on-oke.md#option-1-using-hostnetwork).
...
nodesets:
  ...
  gpu-b4:
    ...
    # hostNetwork only: avoid colliding with the node's own sshd on port 22.
...
Expected: ~189 GB/s avg bus bandwidth on BM.GPU.B4.8 (2 nodes / 16 GPUs).

> With stock `slurmd-rdma` image (no PMIx), use `sbatch` + `mpirun` instead -- see [full guide](deploying-slinky-on-oke.md#running-multi-node-nccl-jobs).

---

## Option B: SR-IOV Virtual Functions (recommended)
...
Expected: ~188 GB/s avg bus bandwidth on BM.GPU.B4.8 (2 nodes / 16 GPUs).

---

## Adapt for Other Shapes

| Shape | `nodeSelector` | `nvidia.com/gpu` | `gres.conf` | `NCCL_IB_HCA` |
|---|---|---|---|---|
| BM.GPU.B4.8 | `BM.GPU.B4.8` | 8 | `Name=gpu Type=a100 File=/dev/nvidia[0-7]` | `mlx5` (SR-IOV) or specific list (hostNetwork) |
| BM.GPU.H100.8 | `BM.GPU.H100.8` | 8 | `Name=gpu Type=h100 File=/dev/nvidia[0-7]` | same |
| BM.GPU.A100-v2.8 | `BM.GPU.A100-v2.8` | 8 | `Name=gpu Type=a100 File=/dev/nvidia[0-7]` | same |
