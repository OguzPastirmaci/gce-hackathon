...
- **Manual GPU GRES** for the current non-NVML worker image. On BM.GPU4.8,
  `AutoDetect=nvidia` was tested and rejected because it invalidated nodes on
  GPU affinity/socket-boundary checks without static topology fields.
...
# ──────────────────────────────────────────────
configFiles:
  # Current validated BM fallback. Use AutoDetect=nvml only with a worker image
  # whose Slurm packages were rebuilt with NVML support.
  gres.conf: |
...
| Shape | `nodeSelector` | `nvidia.com/gpu` | `Gres` | validated `gres.conf` |
|---|---|---|---|---|
...
- One slurmd pod per node means no port conflicts (slurmd uses port 6818)

> **Note:** The slurmd container runs supervisord which starts an internal sshd on port 22. With hostNetwork, this sshd **will fail to bind** because the host's sshd already occupies port 22. This is **non-fatal** -- slurmd communication uses port 6818, and the warning in the slurmd logs can be safely ignored. When using `mpirun` with `-mca plm_rsh_args "-p 22"`, it connects to the **host's** sshd (which works because of hostNetwork), not the slurmd container's sshd.
...
#### Using AutoDetect on Bare Metal

On BM.GPU4.8, plain `AutoDetect=nvidia` in `gres.conf` failed with:
...
This happens because GPU core affinity does not align with Slurm's socket
boundaries on these shapes. In Slinky chart 1.1.0, putting
`Parameters=l3cache_as_socket` under `nodesets.<name>.extraConfMap` is not
sufficient because it is rendered into `slurmd --conf`. Passing
`--parameters l3cache_as_socket` to `slurmd` made Slurm record the parameter,
but the node still failed registration. Do not solve this by statically defining
`Sockets`, `CoresPerSocket`, or `ThreadsPerCore` in the values file.

For this environment, keep the validated manual GRES file until the worker image
is rebuilt with Slurm NVML support:
...
The next autodetect path to test is `AutoDetect=nvml` with a worker image that
contains Slurm's `gpu_nvml.so` plugin.

If a specific shape still fails autodetect, use this manual fallback:
...
NCCL handles GPU/NIC topology at runtime, so Slurm-level GPU core affinity is
not required for the RDMA workload tests in this guide.
...
| **GPU detection (BM)** | Current validated path is manual `Name=gpu Type=<type> File=/dev/nvidia[0-N]`; test `AutoDetect=nvml` only after rebuilding Slurm with NVML support |
...
| **slurmd sshd** | Non-fatal: sshd fails on port 22 (hostNetwork only — no conflict with SR-IOV) |
