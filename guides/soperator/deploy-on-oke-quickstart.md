...
Minimal copy-paste steps for deploying soperator with RDMA on OKE bare-metal GPU nodes in cluster networks.

**Assumes**: `oci-hpc-oke` cluster with CPU pool (`oke-system`) + GPU pool (`oke-rdma`) with NVIDIA GPU Operator, Network Operator, and FSS PV (`fss-pv`) already provisioned. `kubectl` and `helm` working.
...
## 5. Add topology labels to GPU nodes
...
Expected: `Avg bus bandwidth` > 150 GB/s.
